import React from 'react';
import Logout from '../Logout';

const Table = ({ employees, handleEdit, handleDelete,handleView,setIsAdding,setIsAuthenticated }) => {
  employees.forEach((employee, i) => {
    employee.id = i + 1;
  });
  // let new Date() = new Date();
  const formatter = new Intl.NumberFormat('en-US', {
    style: 'currency',
    currency: 'USD',
    minimumFractionDigits: null,
  });

  return (
    <div className='add-container'>
      <div className='d-flex flex-column justify-content-center mr-2 pr-3'>
        <button 
        style={{ backgroundColor: 'white',color:'black',borderColor:'white',fontWeight: 'lighter' }}
        className="p-container"
        onClick={() => setIsAdding(true)}
        >
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-people" viewBox="0 0 16 16">
        <path d="M15 14s1 0 1-1-1-4-5-4-5 3-5 4 1 1 1 1h8Zm-7.978-1A.261.261 0 0 1 7 12.996c.001-.264.167-1.03.76-1.72C8.312 10.629 9.282 10 11 10c1.717 0 2.687.63 3.24 1.276.593.69.758 1.457.76 1.72l-.008.002a.274.274 0 0 1-.014.002H7.022ZM11 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm3-2a3 3 0 1 1-6 0 3 3 0 0 1 6 0ZM6.936 9.28a5.88 5.88 0 0 0-1.23-.247A7.35 7.35 0 0 0 5 9c-4 0-5 3-5 4 0 .667.333 1 1 1h4.216A2.238 2.238 0 0 1 5 13c0-1.01.377-2.042 1.09-2.904.243-.294.526-.569.846-.816ZM4.92 10A5.493 5.493 0 0 0 4 13H1c0-.26.164-1.03.76-1.724.545-.636 1.492-1.256 3.16-1.275ZM1.5 5.5a3 3 0 1 1 6 0 3 3 0 0 1-6 0Zm3-2a2 2 0 1 0 0 4 2 2 0 0 0 0-4Z"/>
        </svg> Add Employee</button>
        <p className='p-container red-container'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-kanban-fill" viewBox="0 0 16 16">
        <path d="M2.5 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2h-11zm5 2h1a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1h-1a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1zm-5 1a1 1 0 0 1 1-1h1a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1h-1a1 1 0 0 1-1-1V3zm9-1h1a1 1 0 0 1 1 1v10a1 1 0 0 1-1 1h-1a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1z"/>
        </svg> Manege Student</p>
        <Logout setIsAuthenticated={setIsAuthenticated} />
      </div>
      <div>
        <div className='upperContainer'>
          <div><h4>Manege Students</h4></div>
          <div className='upperContainer-Add-right'><h4>{new Date().toLocaleString()}</h4></div>
        </div>
        <div className="contain-table">
          <table className="striped-table">
              <tr className='table-header-container'>
                <th colSpan={2}>Id</th>
                <th colSpan={2}>Name</th>
                <th colSpan={2}>Class</th>
                <th colSpan={2}>Roll no</th>
                <th colSpan={2} className="text-center">
                  View / Edit / Delete
                </th>
              </tr>
            <tbody>
              {employees.length > 0 ? (
                employees.map((employee, i) => (
                  <tr key={employee.id}>
                    <td colSpan={2}>{i + 1}</td>
                    <td colSpan={2}>{employee.firstName} {!employee.middleName?" ":employee.middleName} {employee.lastName}</td>
                    <td colSpan={2}>{employee.className}-{employee.divisionName}</td>
                    <td colSpan={2}>{employee.rollNumber}</td>
                    <div className='d-flex flex-row justify-content-center'>
                      <td>
                        <button
                          onClick={() => handleEdit(employee.id)}
                          className="button muted-button"
                        >
                        <i class="material-icons">edit</i>
                        </button>
                      </td>
                      <td>
                        <button
                          onClick={() => handleView(employee.id)}
                          className="button muted-button"
                        >
                        <i class="material-icons">visibility</i>
                        </button>
                      </td>
                      <td>
                        <button
                          onClick={() => handleDelete(employee.id)}
                          className="button muted-button"
                        >
                        <i class="material-icons">delete</i>
                        </button>
                      </td>
                    </div>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan={7}>No Employees</td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
};

export default Table;
